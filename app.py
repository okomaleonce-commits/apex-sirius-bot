...
print("🚀 APEX-SIRIUS vPROD-DEFINITIVE - Filtre temporel intelligent", flush=True)
...
def check_value_bets():
    ...
    for fixture in fixtures[:100]:   # ← plus large pour trouver des pré/live
        try:
            match_date = datetime.fromisoformat(fixture['fixture']['date'].replace('Z', '+00:00'))
            if match_date < now - timedelta(hours=3):  # skip vieux matchs terminés
                continue
        except:
            continue
